...
const fetchMetadata = async (filter_key) => {
  const lambdaClient = new LambdaClient({});
  const payload = JSON.stringify({filter_key: filter_key});
  try {
...
async function retrieveKBDocs(query, knowledgeBase, knowledgeBaseID) {
  ...
  confidenceFilteredResults.map(async (item) => {
    ...
    return {
      title: objectKey + " (Knowledge Base)",
          ...
          if (parsedChunk) {                      
            ...
            if (parsedChunk.type) {
             if (parsedChunk.type == "tool_use") {
               ...
               toolUse['id'] = toolId;
                           // Initialize input based on tool type
                if (parsedChunk.name === "query_db") {
                  toolUse['input'] = {'query': ""};
                } else if (parsedChunk.name === "fetch_metadata") {
                  toolUse['input'] = {'filter_key': ""};
                }
             } 
            }

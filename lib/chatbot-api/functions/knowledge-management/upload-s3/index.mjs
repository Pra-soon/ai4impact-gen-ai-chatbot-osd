  ...
  try {
    const signedUrl = await getSignedUrl(s3, command, {
      expiresIn: URL_EXPIRATION_SECONDS, //Set URL expiration time
        ...
        // Now tag the uploaded file based on the admin role
    let tag = "";
    ...
    if (tag) {
      // Apply the tag to the uploaded file
      const taggingCommand = new PutObjectTaggingCommand({
        Bucket: process.env.BUCKET,
        Key: fileName,
        Tagging: {
          TagSet: [
            { Key: "Category", Value: tag }
          ]
        }
      });
      await s3.send(taggingCommand); // Apply the tag
      console.log(`Tagged file ${fileName} with ${tag}`);
    }
    ...
    return {
      ...
      body: JSON.stringify({ error: 'Failed to generate signed URL or tag file' }),
    };
  }
};
